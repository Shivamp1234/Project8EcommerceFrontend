...
const SmartHome = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/smarthome")
    ...
    return (
        <>
            ...
            <h4 className='headings'>SMART HOME</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 76 && item.id <= 90 }).map((element, index) => (
