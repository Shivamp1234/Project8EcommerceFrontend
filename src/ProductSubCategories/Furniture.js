...
const Furniture = ({ addToCart, count }) => {
    ...
    return (
        <>
            ...
            <h4 className='headings'>FURNITURE</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 181 && item.id <= 195 }).map((element, index) => (
