...
const IndianLanguageBooks = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/indianlanguagebooks")
    ...
    return (
        <>
            ...
            <h4 className='headings'>INDIAN LANGUAGE BOOKS</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 451 && item.id <= 465 }).map((element, index) => (
...
export default IndianLanguageBooks