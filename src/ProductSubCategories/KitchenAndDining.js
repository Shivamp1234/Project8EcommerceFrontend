...
const KitchenAndDining = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/kitchenanddining")
    ...
    return (
        <>
            ...
            <h4 className='headings'>KITCHEN AND DINING</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 166 && item.id <= 180 }).map((element, index) => (
