...
const StorageAndOrganization = ({ addToCart, count }) => {
    ...
    return (
        <>
            ...
            <h4 className='headings'>STORAGE AND ORGANIZATION</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 211 && item.id <= 225 }).map((element, index) => (
...
export default StorageAndOrganization