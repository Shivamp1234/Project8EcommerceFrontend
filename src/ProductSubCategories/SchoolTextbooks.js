...
const SchoolTextbooks = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/schooltextbooks")
    ...
    return (
        <>
            ...
            <h4 className='headings'>SCHOOL TEXTBOOKS</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 466 && item.id <= 480 }).map((element, index) => (
...
export default SchoolTextbooks