...
const KidsFashion = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/kidsfashion")
    ...
    return (
        <>
            ...
            <h4 className='headings'>KIDS FASHION</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 136 && item.id <= 150 }).map((element, index) => (
