...
const PersonalCareAppliances = ({ addToCart, count }) => {
    ...
    return (
        <>
            ...
            <h4 className='headings'>PERSONAL CARE APPLIANCES</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 286 && item.id <= 300 }).map((element, index) => (
