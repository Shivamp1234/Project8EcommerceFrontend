...
const MensFashion = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/mensfashion")
    ...
    return (
        <>
            ...
            <h4 className='headings'>MEN'S FASHION</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 106 && item.id <= 180 }).map((element, index) => (
