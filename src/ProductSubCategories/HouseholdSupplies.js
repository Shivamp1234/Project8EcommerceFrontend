...
const HouseholdSupplies = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/householdsupplies")
    ...
    return (
        <>
            ...
            <h4 className='headings'>HOUSEHOLD SUPPLIES </h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 241 && item.id <= 255 }).map((element, index) => (
