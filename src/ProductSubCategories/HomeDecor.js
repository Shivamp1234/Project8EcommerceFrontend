...
const HomeDecor = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/homedecor")
    ...
    return (
        <>
            ...
            <h4 className='headings'>HOME DECOR</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 196 && item.id <= 210 }).map((element, index) => (
