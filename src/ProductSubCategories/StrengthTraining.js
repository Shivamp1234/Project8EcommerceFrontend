...
const StrengthTraining = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://project-8-ecommerce-backend.onrender.com/products/strengthtraining")
    ...
    return (
        <>
            ...
            <h4 className='headings'>STRENGTH TRAINING</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 391 && item.id <= 405 }).map((element, index) => (
